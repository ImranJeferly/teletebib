...
export default function EditBlogPage() {
    ...
    const loadPost = async () => {
      try {
        ...
        if (post) {
          ...
          alert('Bloq tapılmadı!');
          router.push('/admin');
        }
      } catch (error) {
        console.error('Error loading post:', error);
        alert('Bloq yüklənərkən xəta baş verdi!');
        router.push('/admin');
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      alert(finalStatus === 'draft' ? 'Bloq layihə olaraq yeniləndi!' : 'Bloq uğurla yeniləndi!');
      router.push('/admin');
    } catch (error) {
      console.error('Error updating blog:', error);
      alert(error instanceof Error ? error.message : 'Bloq yenilənərkən xəta baş verdi!');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Bloq yüklənir...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-sm border p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t">
              ...
              >
                <Save className="w-4 h-4 mr-2" />
                {saving ? 'Saxlanır...' : 'Qaralama olaraq saxla'}
              </Button>
              <Button 
                onClick={() => handleSubmit(false)}
                style={{ backgroundColor: PRIMARY }}
